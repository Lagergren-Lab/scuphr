...
do
	...
	job_file="${DATA_DIR}/${EXP_NAME}/dbc_ss_all.sh"
...
#SBATCH -t 24:00:00
#SBATCH -J ${EXP_NAME}
#SBATCH -e ${DATA_DIR}/${EXP_NAME}/dbc_ss_all_err.txt
#SBATCH -o ${DATA_DIR}/${EXP_NAME}/dbc_ss_all_out.txt
...
python3 analyse_dbc_json_single_site.py ${DATA_DIR}/${EXP_NAME}/ --seed_val $SEED_VAL --data_type synthetic --p_ado $TEST_PADO --p_ae $TEST_PAE --print_status 0 --output_dir ${RESULT_DIR}/${EXP_NAME}/${TEST_PADO}_${TEST_PAE}/ 
wait
echo \"$(date) Distance between cells single site is finished\"
...
wait
echo \"$(date) Combine distances single site is finished\"
