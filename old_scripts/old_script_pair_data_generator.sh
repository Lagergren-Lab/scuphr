	...
	do
		...
		do
			...
			do
				...
				job_file="${DATA_DIR}/${EXP_NAME}/pairs_sim.sh"
...
#SBATCH -t 24:00:00
#SBATCH -J ${EXP_NAME}
#SBATCH -e ${DATA_DIR}/${EXP_NAME}/pairs_sim_err.txt
#SBATCH -o ${DATA_DIR}/${EXP_NAME}/pairs_sim_out.txt
...
python3 data_simulation_pairs.py --global_dir ${DATA_DIR}/${EXP_NAME}/ --num_cells $NUM_CELLS --p_ado $DATA_PADO --p_ae $DATA_PAE --mean_coverage $COVERAGE --seed_val $SEED_VAL --avg_mut_per_branch $AVG_MUT --no_mut_to_mut_ratio $MUT_RATIO --chr_id $CHR_ID --read_length $READ_LENGTH
wait
echo \"$(date) Data simulation is finished\"

python3 site_detection.py ${DATA_DIR}/${EXP_NAME}/ $NUM_CELLS --read_length $READ_LENGTH
wait
