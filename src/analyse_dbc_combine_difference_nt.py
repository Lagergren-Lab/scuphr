# This file contains the distance matrix combination code with presence.
# It uses the presence matrices for normalisation. This version is referred as PRESENCE WITH N/T.
# On contrary to analyse_dbc_combine.py,
# this script loads cell-to-cell difference matrix and the corresponding presence matrix.
...
def load_dicts(global_dir, pos_range_min, pos_range_max, data_type):
    ...
    for pos in range(pos_range_min, pos_range_max):
        ...
        filename = global_dir + "matrix/" + str(title_str) + ".out"
        presence_matrix_dict[str(pos)] = np.loadtxt(filename)

    end_time = time.time()
    print("\nTotal time of loading dictionaries: ", end_time - start_time)

    return real_dist_matrix_dict, dist_matrix_dict, presence_matrix_dict


def combine_multiple_sites(global_dir, real_dist_matrix_dict, dist_matrix_dict, presence_matrix_dict, num_cells,
                           pos_range_min, pos_range_max, data_type):
    start_time = time.time()
    ...
    for pos in range(pos_range_min, pos_range_max):
        ...
        # He site is heterozygous
            ...
            # Instead of 1, add presence
            temp_pres = presence_matrix_dict[str(pos)]
            inferred_mask = np.multiply(inferred_mask, np.nan_to_num(presence_matrix_dict[str(pos)]))
            np.fill_diagonal(inferred_mask, 1)
            ...
            if data_type == "synthetic":  # If the dataset is synthetic
                ...
                plt.xlabel("Cells")
                plt.ylabel("Cells")
                title_str = "matrix_real_difference_pos_so_far_" + str(sub)
                plt.title(title_str)
                ...
                plt.xlabel("Cells")
                plt.ylabel("Cells")
                title_str = "matrix_infer_difference_pos_so_far_" + str(sub)
                plt.title(title_str)
    ...
    if data_type == "synthetic":  # If the dataset is synthetic
        ...
        np.savetxt(filename, real_res_sub, delimiter=',', fmt="%.10f")
        filename = global_dir + "real_dif_count.out"
        np.savetxt(filename, real_res_mask)

    filename = global_dir + "infer_dif.csv"
    np.savetxt(filename, res_sub, delimiter=',', fmt="%.10f")
    ...
    if data_type == "synthetic":  # If the dataset is synthetic
        ...
        plt.xlabel("Cells")
        plt.ylabel("Cells")
        title_str = "matrix_real_difference"
        plt.title(title_str)
        ...
        plt.xlabel("Cells")
        plt.ylabel("Cells")
        title_str = "matrix_infer_difference"
        plt.title(title_str)
...
def main():
    ...
    real_dist_matrix_dict, dist_matrix_dict, presence_matrix_dict = load_dicts(args.output_dir, args.pos_range_min,
                                                                               args.pos_range_max, args.data_type)

    print("\n*****\nCombining multiple sites")
    start_time = time.time()
    combine_multiple_sites(args.output_dir, real_dist_matrix_dict, dist_matrix_dict, presence_matrix_dict, num_cells,
                           args.pos_range_min, args.pos_range_max, args.data_type)
    end_time = time.time()
