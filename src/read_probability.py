...
from fragment_enumerator import enumerate_fragments_possible
...
def convert_fragment_lambda_key_format(fragment_lambda_key):
    ...
    fragment_key = fragment_lambda_key[0:17]
    lambdas_key = fragment_lambda_key[18:]
...
def precompute_reads(cell_list, z_list, bulk, print_results=False):
    ...
    for z_ind in range(len(z_list)):
        ...
        z = z_list[z_ind]
        fragment_list = enumerate_fragments_possible(np.array(bulk).astype(int), np.array(z).astype(int))
...
def read_probability(reads, p_error, fragments):
    ...
    if all(fragments[0] == fragments[1]) and all(fragments[0] == fragments[2]):
        # print("\tall same")
        max_j = 1
        max_k = 1
    elif all(fragments[0] == fragments[1]):
        # print("\tfirst and second same")
        max_j = 1
    elif all(fragments[0] == fragments[2]):
        # print("\tfirst and third same")
        max_k = 1
    elif all(fragments[1] == fragments[2]):
