...
def analyse_infer_dbc_one_pos(pos, output, dataset, p_ae, p_ado, a_g, b_g, alpha_list, data_type, read_prob_dir,
    ...
    if os.path.exists(log_zcy_filename) and os.path.getsize(log_zcy_filename) > 0:
        ...
        if os.path.exists(log_zcy_pos_filename) and os.path.getsize(log_zcy_pos_filename) > 0:
            ...
            log_zcy_dict_prior = compute_zcy_log_dict_pos(dataset, read_dicts, p_ado, p_ae, print_results)
        ...
        for j in range(num_cells):  # num_cells
            if i <= j:
                ...
                if cell_list[i].lc > 0 and cell_list[j].lc > 0:
                    ...
                    print("\n Similarity. Cells with reads for pos: ", pos, " are ids: ", i, j, " dist: ", same_prob)
...
def analyse_infer_dbc_one_pos_pool(pos, dataset, p_ae, p_ado, a_g, b_g, alpha_list, data_type, read_prob_dir,
    ...
    is_correct = False
    if data_type == "synthetic":  # If the data is synthetic
        if dataset['common_z'][0][0] == highest_z[0][0] and dataset['common_z'][0][1] == highest_z[0][1] and \
                dataset['common_z'][1][0] == highest_z[1][0] and dataset['common_z'][1][1] == highest_z[1][1]:
            is_correct = True
...
def main():
    ...
    dataset = json.load(open(proc_dict_dir + "data.txt"))
    ...
    if args.data_type == "synthetic":
        ...
        if False:  # Parallelised via Process
            output = mp.Queue()
            processes = [mp.Process(target=analyse_real_dbc_one_pos,
                                    args=(int(pos), output, dataset[str(pos)], matrix_dir, print_results)) for pos in
                         positions]
            ...
            while 1:
                ...
                while not output.empty():
                    real_results.append(output.get())
                if not running:
                    break
    ...
    if False:  # Parallelised via Process
        output = mp.Queue()
        processes = [mp.Process(target=analyse_infer_dbc_one_pos, args=(
        int(pos), output, dataset[str(pos)], args.p_ae, args.p_ado, args.a_g, args.b_g, alpha_list, args.data_type,
        read_prob_dir, matrix_dir, common_z_dir, print_results)) for pos in positions]
        # processes = [mp.Process(target=analyseInferDBC_onepos, args=(int(pos), output, dataset[str(pos)],
        # read_dicts[int(pos)], args.p_ae, args.p_ado, args.a_g, args.b_g, alpha_list, args.data_type, read_prob_dir,
        # matrix_dir, common_z_dir, print_results)) for pos in positions]
        ...
        while 1:
            running = any(p.is_alive() for p in processes)
            while not output.empty():
                infer_results.append(output.get())
            if not running:
                break
