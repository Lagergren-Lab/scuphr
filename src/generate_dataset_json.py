    ...
    for pos_pair in bulk_pair_dict.keys():
        ...
        if cell_count >= min_cell_count:
            ...
            for cell_name in cell_name_list:
                if pos_pair in all_cell_dict_pair[cell_name].keys():
                    ...
                    for i_ in range(q_error.shape[0]):
                        try:
                            ...
                            if q_error_chars[i_] in phred_chars:
                                q_error[i_] = phred_chars.index(q_error_chars[i_])
                            else:
                                # chr conversion is added due to the differences in rust data simulation
                                # q_error[i_] = phred_chars.index(chr(q_error_chars[i_]))
                                q_error[i_] = q_error_chars[i_]
                ...
                if data_type == 'synthetic':
                    ...
                    # If singleton
                        ...
                        hom_pos = int(pos_pair)

                        if cell_genome[hom_pos - 1, 0] == bulk_genome[hom_pos - 1, 0] \
                                and cell_genome[hom_pos - 1, 1] == bulk_genome[hom_pos - 1, 1]:
                            cell_mut_status = 0
                        else:
                            cell_mut_status = 1
                            tot_mut_count += 1
                        ...
                        bulk = np.array([bulk_genome[hom_pos - 1, 0], bulk_genome[hom_pos - 1, 1]])
                    ...
                    if "_" in pos_pair:
                        ...
                        bulk = np.array([bulk_pair_dict[pos_pair], bulk_pair_dict[pos_pair]])
            ...
            try:
                ...
                z_list = []
                for i in range(3):
                    z_list.append([int(bulk[0]), int((bulk[0] + i + 1) % 4)])
            ...
            if data_type == 'synthetic':
                ...
                if hom_pos - 1 not in mut_locations:
                    ...
                    try:
                        ...
                        z = mut_genotype[hom_pos - 1]
                        pos_dict['pos_pair'] = hom_pos  # pos_pair
            else:
                ...
                if "_" in pos_pair:
                    ...
                    pos_dict['pos_pair'] = int(pos_pair)  # pos_pair
...
def main():
    ...
    for cell_idx in range(args.num_cells):
        ...
        cell_out_filename = proc_dir + "cell_" + str(cell_idx) + ".pickle"
