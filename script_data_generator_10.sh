...
python3 data_simulation_pairs.py --global_dir ${DATA_DIR}/${EXP_NAME}/ --num_cells $NUM_CELLS --p_ado $DATA_PADO --p_ae $DATA_PAE --mean_coverage $COVERAGE --seed_val $SEED_VAL --avg_mut_per_branch $AVG_MUT --chr_id $CHR_ID --read_length $READ_LENGTH --phased_freq $PHASED_FREQ --genome_length $GENOME_LENGTH --cnv_ratio $CNV_RATIO
wait
